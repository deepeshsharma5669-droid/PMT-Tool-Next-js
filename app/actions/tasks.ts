...
export async function assignTask(taskId: string, assigneeId: string): Promise<ActionResult<null>> {
  try {
    await requireTaskManagerForTask(taskId)
  } catch (error) {
    return fail(error)
  }
  const supabase = await createClient()
  const { error } = await supabase.rpc('pmt_assign_task', {
    p_task_id: taskId,
    p_assignee_id: assigneeId,
  })
  if (error) return fail(error)
...
export async function updateTask(taskId: string, title: string, description: string): Promise<ActionResult<null>> {
  try {
    await requireTaskManagerForTask(taskId)
  } catch (error) {
    return fail(error)
  ...
  const supabase = await createClient()
  const { error } = await supabase.rpc('pmt_update_task', {
    p_task_id: taskId,
    p_title: title,
    p_description: description,
  })
...
export async function changeDeadline(taskId: string, deadline: string): Promise<ActionResult<null>> {
  try {
    await requireTaskManagerForTask(taskId)
  } catch (error) {
    return fail(error)
  ...
  const supabase = await createClient()
  const { error } = await supabase.rpc('pmt_change_deadline', {
    p_task_id: taskId,
    p_deadline: deadline,
  })
  if (error) return fail(error)
